				...
				bool update_i_size)
{
	...
	if (compress_type != BTRFS_COMPRESS_NONE) {
		...
		struct folio *folio;
		folio = filemap_lock_folio(inode->vfs_inode.i_mapping, 0);
		if (IS_ERR(folio)) {
			ret = PTR_ERR(folio);
			goto fail;
		}
...
static void compress_file_range(struct btrfs_work *work)
{
	...
	ret = btrfs_compress_bio(compress_type | (fs_info->compress_level << 4),
				 &async_extent->bbio->bio, pages, &nr_pages);
	if (ret < 0)
	...
	async_extent->compressed_size = total_compressed;
...
static int submit_async_extent(struct async_extent *async_extent)
{
	...
	ret = btrfs_reserve_extent(root, len,
...
out:
	 ...
	 */
	if (extent_reserved) {
		clear_extent_bits(&inode->io_tree, start,
 ...
 */
static noinline void submit_compressed_extent(struct btrfs_work *work, bool do_free)
{
	struct async_extent *async_extent = container_of(work, struct async_extent,
						     work);
	...
	if (do_free) {
		if (async_extent->blkcg_css)
			css_put(async_extent->blkcg_css);
		kfree(async_extent);
		return;
	}

	nr_pages = (async_extent->end - async_extent->start + PAGE_SIZE) >>
		PAGE_SHIFT;

	/*
	 * ->inode could be NULL if async_extent_start has failed to compress,
	 * in which case we don't have anything to submit, yet we need to
	 * always adjust ->async_delalloc_pages as its paired with the init
	 * happening in run_delalloc_compressed
	 */
	if (async_extent->inode)
		submit_async_extent(async_extent);
		...
		struct bio *bio, struct writeback_control *wbc)
{
	...
	nofs_flag = memalloc_nofs_save();
	async_extent = kzalloc(sizeof(*async_extent), GFP_KERNEL);
	memalloc_nofs_restore(nofs_flag);
	if (!async_extent) {
	...
	if (blkcg_css != blkcg_root_css) {
		css_get(blkcg_css);
		async_extent->write_flags |= REQ_BTRFS_CGROUP_PUNT;
		async_extent->blkcg_css = blkcg_css;
	} else {
		async_extent->blkcg_css = NULL;
	}

	btrfs_init_work(&async_extent->work, compress_file_range,
			submit_compressed_extent);
			   ...
			   const u64 start, const u64 end)
{
	 ...
	 *    clear_extent_bits()) the bytes_may_use counter of the
...
must_cow:
		...
		clear_extent_bits(&inode->io_tree, cur_offset, nocow_end,
					     EXTENT_DELALLOC |
...
error:
	...
	if (cur_offset < end)
		clear_extent_bits(&inode->io_tree, cur_offset, end,
		...
		struct inode *inode, loff_t offset)
{
	...
	if (ordered) {
		if (offset < ordered->writeback_end)
	...
	if (ordered->writeback_end == ordered->file_offset)
		ordered->writeback_end = min(ordered->file_offset + ordered->num_bytes, bwb->end + 1);
	btrfs_oe_to_iomap(inode, ordered, &wpc->iomap);
...
static struct btrfs_writeback *__btrfs_writepages(struct inode *inode, struct writeback_control *wbc, struct extent_state *cached_state)
{
	int ret = 0;
	struct btrfs_writeback *bwb;
	 ...
	 */
	if (!filemap_range_needs_writeback(inode->i_mapping, wbc->range_start, wbc->range_end)) {
		unlock_extent(&BTRFS_I(inode)->io_tree, wbc->range_start, wbc->range_end, &cached_state);
		return NULL;
	}
			    ...
			    struct writeback_control *wbc)
{
	...
	do {
		...
		bwb = __btrfs_writepages(inode, wbc, cached);
		if (!IS_ERR_OR_NULL(bwb))
			btrfs_writeback_put(bwb);
