...
static noinline int test_btrfs_get_extent(u32 sectorsize, u32 nodesize)
{
	...
	em = btrfs_get_extent(BTRFS_I(inode), NULL, 0, sectorsize, NULL);
	if (IS_ERR(em)) {
		em = NULL;
	...
	em = btrfs_get_extent(BTRFS_I(inode), NULL, 0, (u64)-1, NULL);
	if (IS_ERR(em)) {
		test_err("got an error when we shouldn't have");
		goto out;
	}
	...
	em = btrfs_get_extent(BTRFS_I(inode), NULL, offset, sectorsize, NULL);
	if (IS_ERR(em)) {
		test_err("got an error when we shouldn't have");
		goto out;
	}
	...
	em = btrfs_get_extent(BTRFS_I(inode), NULL, offset + 6, sectorsize, NULL);
	if (IS_ERR(em)) {
		test_err("got an error when we shouldn't have");
		goto out;
	}
	...
	em = btrfs_get_extent(BTRFS_I(inode), NULL, offset, SZ_4M, NULL);
	if (IS_ERR(em)) {
		test_err("got an error when we shouldn't have");
		goto out;
	}
...
static int test_hole_first(u32 sectorsize, u32 nodesize)
{
	...
	em = btrfs_get_extent(BTRFS_I(inode), NULL, 0, 2 * sectorsize, NULL);
	if (IS_ERR(em)) {
		test_err("got an error when we shouldn't have");
		goto out;
	}
	...
	em = btrfs_get_extent(BTRFS_I(inode), NULL, sectorsize, 2 * sectorsize, NULL);
	if (IS_ERR(em)) {
		test_err("got an error when we shouldn't have");
		goto out;
	}
